...
# kpi_full_pipeline_combined.py
# This script combines the best features from data_analyst_13.py and data_analyst_8.py
# All comments and text have been translated to English.
...
# Pareto Analysis (80/20 rule) - from data_analyst_8.py
...
pareto_threshold_80 = product_revenue[product_revenue["Cumulative_Revenue_pct"] >= 0.8].iloc[0]["Product_ID"]
print(f"[INFO] Pareto Analysis (80/20 Rule): 80% of revenue comes from products up to {pareto_threshold_80}")
...
forecast_results = []
top_ids_forecast = top_ids[:2] # Forecasting only for the top 2 products for brevity
...
for pid in top_ids_forecast:
    ...
    forecast_results.append({"Product_ID": pid, "Model": "LinearRegression", "rmse": rmse, "mae": mae, "mape_pct": mape})
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(forecast_dir, f"forecast_linreg_{pid}.png"))
    plt.close()

# --- Forecasting with Prophet ---
# for pid in top_ids_forecast:
    # prophet_df = df[df["Product_ID"] == pid][["Date", "Sales_Units"]].rename(columns={"Date": "ds", "Sales_Units": "y"})
    # if len(prophet_df) < 30:
        # continue
    # model_p = Prophet(daily_seasonality=False, weekly_seasonality=True, yearly_seasonality=True, changepoint_prior_scale=0.05)
    # model_p.fit(prophet_df)
    # future = model_p.make_future_dataframe(periods=FORECAST_H, freq='W')
    # forecast = model_p.predict(future)
    # y_test = prophet_df["y"].values[-FORECAST_H:]
    # y_pred = forecast["yhat"].values[-FORECAST_H:]
    # rmse = sqrt(mean_squared_error(y_test, y_pred))
    # mae = mean_absolute_error(y_test, y_pred)
    # mask = y_test != 0
    # mape = np.mean(np.abs((y_test[mask] - y_pred[mask]) / y_test[mask])) * 100 if mask.sum() > 0 else np.nan
    # forecast_results.append({"Product_ID": pid, "Model": "Prophet", "rmse": rmse, "mae": mae, "mape_pct": mape})
    # fig_p = model_p.plot(forecast); plt.title(f"Prophet Forecast - {pid}"); plt.savefig(os.path.join(forecast_dir, f"forecast_prophet_{pid}.png")); plt.close(fig_p)

# --- Forecasting with ARIMA ---
for pid in top_ids_forecast:
    arima_df = df[df["Product_ID"] == pid][["Date", "Sales_Units"]].set_index("Date").resample('W').sum().fillna(0)
    if len(arima_df) < 60:
        continue
    try:
        model_a = sm.tsa.statespace.SARIMAX(arima_df["Sales_Units"], order=(1,1,1), seasonal_order=(1,1,1,12), enforce_stationarity=False, enforce_invertibility=False)
        results_a = model_a.fit()
        forecast_a = results_a.get_forecast(steps=FORECAST_H)
        y_test = arima_df["Sales_Units"].iloc[-FORECAST_H:].values
        y_pred = forecast_a.predicted_mean.values
        rmse = sqrt(mean_squared_error(y_test, y_pred))
        mae = mean_absolute_error(y_test, y_pred)
        mask = y_test != 0
        mape = np.mean(np.abs((y_test[mask] - y_pred[mask]) / y_test[mask])) * 100 if mask.sum() > 0 else np.nan
        forecast_results.append({"Product_ID": pid, "Model": "ARIMA", "rmse": rmse, "mae": mae, "mape_pct": mape})
        plt.figure(figsize=(10, 4))
        plt.plot(arima_df.index, arima_df["Sales_Units"], label="Historical")
        plt.plot(forecast_a.predicted_mean.index, y_pred, label="ARIMA Forecast", marker='o')
        plt.title(f"ARIMA Forecast - {pid}")
        plt.xlabel("Date"); plt.ylabel("Sales Units")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(forecast_dir, f"forecast_arima_{pid}.png"))
        plt.close()
...
ax1.set_ylabel("WoW % (sales)"); ax1.set_xlabel("Date")
fig.tight_layout(); fig.savefig(os.path.join(OUT_DIR, "company_weekly_trends_and_wow.png")); plt.close(fig)

# Monthly revenue & profit line
...
plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "monthly_revenue_profit.png")); plt.close(fig)

# Quarterly Efficiency & Lead Time
...
fig.tight_layout(); fig.savefig(os.path.join(OUT_DIR, "quarterly_efficiency_leadtime.png")); plt.close(fig)

# Region performance & Acquisition rate
...
axes[0].set_title("Revenue by Region"); axes[0].set_xlabel("Region"); axes[0].set_ylabel("Revenue")
axes[1].bar(region_perf["Region"], region_perf["Acquisition"])
axes[1].set_title("Average Acquisition by Region"); axes[1].set_xlabel("Region"); axes[1].set_ylabel("Acquisition")
fig.tight_layout(); fig.savefig(os.path.join(OUT_DIR, "region_performance.png")); plt.close(fig)

# Campaign revenue pie chart
...
plt.title("Campaign Revenue Share"); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "campaign_revenue_share_pie.png")); plt.close(fig)
...
plt.title("Correlation Matrix (Monthly KPIs)"); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "correlation_matrix_monthly.png")); plt.close(fig)
...
plt.title("Heatmap: MoM % Change - Products x Months"); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "mom_heatmap_products_months.png")); plt.close(fig)
...
if top_ids:
    ...
    if len(t0) > 5:
        ...
        plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, f"eff_vs_leadtime_{pid0}.png")); plt.close(fig)
...
plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "anomalies_revenue.png")); plt.close(fig)
...
plt.xlabel("Weeks of Inventory"); plt.ylabel("Frequency"); plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "weeks_inventory_hist.png")); plt.close(fig)
...
with PdfPages(report_path) as pdf:
    ...
    # add a page with 8 actionable recommendations
    fig = plt.figure(figsize=(11, 8.5)); plt.axis("off")
    recs = [
        ...
        "3) Use weekly forecasts (LinearRegression, Prophet, ARIMA) for inventory planning and raw material procurement (Raw_Material_Cost).",
