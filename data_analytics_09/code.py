...
"""
End-to-end KPI analysis for cosmetics/hygiene products.
- Generates (if missing) a 3-year weekly dataset for 12 SKUs
- Computes operational KPIs and time-based metrics (WoW, MoM, QoQ, YoY)
- Performs segmentation, anomaly detection, and forecasting (LinearRegression)
- Saves CSV outputs, PNG plots (one figure per file), and a PDF management report
Notes:
- Report text is Persian; KPI names and technical terms are included in English parentheses.
- Forecasting uses sklearn.linear_model.LinearRegression only.
"""
    ...
    for p in products:
        ...
        for d in dates:
            ...
            rows.append({
                ...
                "Acquisition_Rate": acquisition, "Retention_Rate": round(retention,4), "NPS": round(nps,1),
...
ax0.set_title("روند هفتگی درآمد و فروش (Weekly Revenue & Sales)")
ax0.legend()
...
plt.title("روند ماهانه: Revenue & Profit (monthly)")
...
region_perf = df.groupby("Region").agg({"Revenue":"sum","Profit":"sum","Acquisition_Rate":"mean"}).reset_index()
fig, axes = plt.subplots(1,2, figsize=(14,6))
axes[0].bar(region_perf["Region"], region_perf["Revenue"])
axes[0].set_title("Revenue by Region")
axes[0].set_xlabel("Region"); axes[0].set_ylabel("Revenue")
axes[1].bar(region_perf["Region"], region_perf["Acquisition_Rate"])
...
with PdfPages(report_path) as pdf:
    ...
    plt.text(0.5,0.6,"گزارش مدیریتی KPI محصولات آرایشی/بهداشتی", fontsize=20, ha='center')
    ...
    # summary table text
    fig = plt.figure(figsize=(11,8.5)); plt.axis("off")
    plt.text(0.01,0.98,"خلاصه: Top products by Revenue (last year)", fontsize=14, va='top')
    ...
    fig = plt.figure(figsize=(11,8.5)); plt.axis("off")
    recs = [
        "1) تمرکز تبلیغات روی محصولات 20% بالای فروش (Pareto) — monitor Sales_Units & Revenue.",
        "2) کاهش Defect_Rate و Downtime_hours با برنامه نگهداری پیشگیرانه — افزایش Efficiency_u_per_h.",
        "3) استفاده از پیش‌بینی هفتگی (LinearRegression) برای برنامه‌ریزی موجودی و خرید مواد خام (Raw_Material_Cost).",
        "4) زمان‌بندی پروموشن‌ها با ارزیابی MoM و YoY اثرات کمپین‌ها (Campaign).",
        "5) کاهش Weeks_of_Inventory برای SKUs با گردش سریع و افزایش برای فصلی‌ها.",
        "6) بررسی آنومالی‌ها (anomaly) قبل از تصمیم‌گیری؛ علت‌یابی (logistics/returns/campaign).",
        "7) گزارش هفتگی خودکار با KPI های کلیدی: Sales_Units, Revenue, Profit, Lead_Time_days, Efficiency_u_per_h.",
        "8) تست A/B برای کمپین‌ها و ارزیابی CAC و Conversion (در کانال‌های مختلف)."
    ]
    plt.text(0.01,0.98,"Recommendations (خلاصه):", fontsize=14, va='top')
